...
\label{cha:the_artificial_neural_network}
	...
	\label{sec:the_loss_function}
		...
		types of Loss functions, for our network we have decided to use two well known functions, varying on the
		...
		task. The MSE is obtained with the formula
		...
		\begin{equation*}
		     \nabla_{\hat{\mathbf{y}}}L\left(\hat{\mathbf{y}}, \mathbf{y}\right) = \nabla_{\hat{\mathbf{y}}}
		     \frac{1}{2}\sum_{i = 1}^{m}\left ( \hat{\mathbf{y}} - \mathbf{y} \right )^{2}_{i}
		\end{equation*}
