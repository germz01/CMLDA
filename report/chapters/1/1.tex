	...
	\label{sec:the_loss_function}
        ...
        \label{sub:properties_of_the_loss_function}
            ...
            \begin{itemize}
                ...
                Futhermore, a differentiable function is said to have an L-Lipschitz continuous gradient if, for some $L>0$ holds: 

                \begin{equation*}
                    || \nabla f(\mathbf{x}) - \nabla f(\mathbf{y}) || \le L(|| \mathbf{x} - \mathbf{y}||) \quad \forall (\mathbf{x}, \mathbf{y}), \ \in \ \mathbf{dom} \ f.
                \end{equation*}

                Since the Loss function is bounded in the interval dectated by the logistic function, we can say that the gradient of the function is also bounded in the same interval, so the MSE has an L-Lipschitz continuous gradient.
