...
\label{cha:optimizers}
	...
	\label{sec:sgd}
		...
		\label{sec:momentum}
			When computing the adjustment of the synaptic weights $\Delta\textbf{W}_k$ as in Eq. \ref{delta}, the choice of the learning rate $\eta$ influences the convergence of the SGD algorithm.
			...
			It consists in the adjustment of the new weigths through a velocity vector \textbf{v} that accumulates the gradient elements in the directions of reduction of the loss function \ref{loss} and a momentum coefficient $\alpha \in [0,1]$: the larger is  $\alpha$, the more previous gradients affect the current direction.
			...
			A variant of the CM algorithm, is the \textit{Nesterov's Accelerated Gradient} (NAG), which allows to avoid the oscillatory behaviour visible in the trajectory computed with CM, as visible in fig.\ref{fig:momentum_graph}%fig2 on the importance}. 
			...
			It's worth to underline that, in case of batch choice and convex functions, Nesterov momentum brings the rate of convergence from $O(\frac{1}{k})$ (after k steps) to $O(\frac{1}{k^2})$ % Nesterov (1983).
			\begin{figure}
			\centering
		...
		\label{sec:regularization}
			...
			Regularization, infact, adds the penalty as model complexity increases, forcing some weights to take values close to zero, if they have little influence on the newtork performance and so resulting in poor generalization.
			...
			where $\mathcal{E}_c(\textbf{W})$ is a complexity penalty term measured in term of the weights, and $\lambda$ is a parameter which tells how much importance has the complexity penalty term.
