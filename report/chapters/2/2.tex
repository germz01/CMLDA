...
%deeplearning
		...
		Unfortunatly, as we showed in Sec.\ref{sub:properties_of_the_loss_function}, our objective function is not convex, so we can't rely on what has been written before. Anyway, this issue seems to be overcomed: as proved in \cite{1606.04838}, the SGD is convergent even in presence of nonconvex functions.
		If the algorithm is run with a stepsize sequence satisfying Eq.\ref{delta} and the following assumptions holds:

		\begin{asu} - \label{as:sgd_conv1}
				The Loss function \textit{L} is continuously differentiable and the gradient function $\nabla\textit{L}$ is Lipschitz continuous with Lipschitz constant $B > 0$ s.t. $\|\nabla\textit{L}(\textbf{W})-\nabla\textit{L}(\widetilde{\textbf{W}})\| \leq B\|\textbf{W}-\widetilde{\textbf{W}}\| , \forall \textbf{W},\widetilde{\textbf{W}} \in \mathcal{N}$ ;
				%ensures that the gradient of F does not change arbitrarily quickly with respect to the parameter vector
		\end{asu}
		\begin{asu} - \label{as:sgd_conv2}
				...
				\begin{itemize}
					...
					\item There exist scalars $\mu_G \geq \mu > 0 $ s.t., $\forall k \in \mathbb{N}$,
						\begin{equation}
							\nabla\textit{L}(\textbf{W}_k)^T\mathbb{E}_{\xi_k}[g(\textbf{W}_k,\xi_k)] \geq \mu \|\nabla\textit{L}(\textbf{W}_k)\|^2 \text{ and }
						\end{equation}
						\begin{equation}
							\|\mathbb{E}_{\xi_k}[g(\textbf{W}_k,\xi_k)]\|\leq \mu_G \|\nabla\textit{L}(\textbf{W}_k)\|,
						\end{equation}
		...
		\label{sec:regularization}
			...
			\begin{mini}
			\label{eq:reg}
			  {\textbf{W}\in \mathbb{R}^n}{\textit{L}(\textbf{W}) + \lambda\Omega(\textbf{W}),}{}{}
			\end{mini}
			...
			The addition of squared terms (or of the sum of absolute values) to the Loss function of Eq.\ref{mse}, returns again a continous and differentiable function.%Stochastic Gradient Descent and Regularization - Stanford CS Theory.Tim Roughgarden  Gregory Valiant
	...
	\label{sec:nonlinear_conjugate_gradient}
		...
		\label{sub:beta}
			...
			\begin{asu} - \label{as:cg_conv}
  				If the level set $\mathcal{L} = \{w : \textit{L}(\textbf{W}) \leq L(\textbf{W}_1)\}$ is bounded, in some neighborhood $\mathcal{N}$ of $\mathcal{L}$ the objective function \textit{L} is continuously differentiable, and its gradient is Lipschitz continuous, there exist a constant $B > 0$ s.t. $\|\nabla\textit{L}(\textbf{W})-\nabla\textit{L}(\widetilde{\textbf{W}})\| \leq B\|\textbf{W}-\widetilde{\textbf{W}}\| , \forall \textbf{W},\widetilde{\textbf{W}} \in \mathcal{N}$ ;
			\end{asu}
